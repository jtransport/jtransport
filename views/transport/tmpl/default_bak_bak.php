...
<script type="text/javascript">
	...
	$(function () {
		...
		$('#one-step-progress').on("positionChanged", function (e) {
			viewModelOneStep.percent(e.percent);
		});

		/*ko.applyBindings(viewModelOneStep);*/
		...
		$('#all-steps-progress').on("positionChanged", function (e) {
			viewModelAllSteps.percent(e.percent);
		});

		ko.applyBindings(viewModelAllSteps);
	});

    var viewModelOneStep = {
	    ...
	    stepIt: function () {
		    $('#one-step-progress').progressbar('stepIt');
	    },

	    reset: function () {
		    $('#one-step-progress').progressbar('reset');
	    },
    ...
    viewModelOneStep.formattedPercent = ko.computed(function () {
	    return this.percent() + '%';
    }, viewModelOneStep);

	var viewModelAllSteps = {
		percent: ko.observable(0),
		step: ko.observable(1),
		maximum: ko.observable(100),
		position: ko.observable(80),
	...
	viewModelAllSteps.formattedPercent = ko.computed(function () {
		return this.percent() + '%';
	}, viewModelAllSteps);
